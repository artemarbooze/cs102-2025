...
def remove_wall(grid: List[List[Union[str, int]]], coord: Tuple[int, int]) -> List[List[Union[str, int]]]:
    """
    Remove wall between cells using random direction.
...
def bin_tree_maze(rows: int = 15, cols: int = 15, random_exit: bool = True) -> List[List[Union[str, int]]]:
    ...
    :param random_exit: Whether to generate random exits
...
def get_exits(grid: List[List[Union[str, int]]]) -> List[Tuple[int, int]]:
    """
    Find all exit coordinates in the maze.
...
def make_step(grid: List[List[Union[str, int]]], k: int) -> List[List[Union[str, int]]]:
    """
    Perform one step of BFS wave propagation.
...
def shortest_path(
    ...
    """
    Find shortest path from exit to entrance.

    :param grid: The solved maze grid
    :param exit_coord: Exit coordinate
    :return: List of coordinates forming the path, or None
    """
    path = []
    current = exit_coord
    rows, cols = len(grid), len(grid[0])
...
def solve_maze(
    ...
    """
    Solve the maze and find the path from entrance to exit.

    :param grid: The maze grid
    :return: Tuple of (solved grid with distances, path)
    """
    grid = deepcopy(grid)
...
def add_path_to_grid(
    ...
    :param grid: The maze grid
    :param path: List of coordinates forming the path
